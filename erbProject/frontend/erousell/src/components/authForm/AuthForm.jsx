...
import { Paper, TextField, InputAdornment, IconButton, Typography, Button, Grid, Checkbox, FormGroup, FormControlLabel, Box, Alert } from '@mui/material';
...
const AuthForm = (props) => {
  ...
  return (
    <Box className={styles.loginCard}  component="form" onSubmit={handleSubmit} onChange={handleChange}>
      <Grid container className={styles.fieldsContainer} rowSpacing={2}>
        {
          type === 'register' && (
            <Grid item>
              <TextField label="Username" name="userName" variant="outlined" size='small' color='secondary' sx={{width: '100%'}} />
            </Grid>
          )
        }
        <Grid item>
        ...
        </Grid>
        {
          type === 'register' && (
            <Grid item>
              ...
              />
              {
                !validPassword && <div className={styles.err}>
        ...
        }
        {
          type === "login" ? (
            ...
            <Grid className={styles.termsAndPrivacy}>
              ...
              <span>I agree to receive promotions and marketing messages from Erousell and its partners</span>
            </Grid>
          )
        }
        <Grid item className={valid? `${styles.submitBtn}` : `${styles.submitBtn} ${styles.disabled}`}>
          <Button type="submit" variant="contained" disabled={!valid} sx={{width: "100%"}} className={styles.submit}>{type === "login" ? 'Login' : 'Sign up'}</Button>
        </Grid>
      </Grid>
      <Grid container className={styles.otherSection}>
        {
          type === 'login' ? (
            <Grid item className={styles.redirectRegister}>
              ...
              <div style={{cursor:"pointer"}} onClick={()=>{setOpen(false); setRegisterOpen(true)}}>
                Sign Up
              </div>
            </Grid>
          ) : (
            <Grid item className={`${styles.redirectRegister} ${styles.redirect}`}>
              <span>
                Have an account?
              </span>
              <div style={{cursor:"pointer"}} onClick={()=>{setOpen(false); setLoginOpen(true)}}>
                Log in now
              </div>
            </Grid>
          )
